...
class SalesRepository :
    ...
    def get_sale_row(self, id : int) :
        sale_row = self.__connector.execute("SELECT * FROM Sales WHERE id = ?", [id]).fetchone()
        return SaleRow(sale_row[0], sale_row[1], sale_row[2], sale_row[3], sale_row[4])
    
    def get_all_sale_rows(self, user_id : int = None) :
        if user_id :
            sale_rows = self.__connector.execute("SELECT * FROM Sales WHERE User_id = ?", [user_id])
        else :
            sale_rows = self.__connector.execute("SELECT * FROM Sales")
        output = []
        for s in sale_rows :
            output.append(SaleRow(s[0], s[1], s[2], s[3], s[4]))
        return output
    ...
    def total_amount(self, user_id : int) :
        total = self.__connector.execute("SELECT SUM(Amount) FROM Sales WHERE User_id = ?", [user_id]).fetchone()
